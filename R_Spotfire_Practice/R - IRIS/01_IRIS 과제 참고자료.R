...
##가로 축은 Sepal.Length로, 세로축은 Sepal.Width로 설정, geom_point로 설정.
ggplot(iris, aes(Sepal.Length, Sepal.Width))  + geom_point()

##색상추가
ggplot(iris, aes(Sepal.Length, Sepal.Width))  + geom_point(aes(colour = Species))

##point의 크기는 꽃잎의  넓이(Petal.Length)에 따라 설정
...
## Notice the petal length of the setosa is clearly a differenciated cluster 
...
## Versicolor species is more mixed, this feature might not be a good predictor.
...
## The examination of the plot revelas a strong correlation between 
## the variables Petal Width and the Petal Length (96%) as well as
## The Sepal Length and Petal Length (87%).
## The heatmat is another useful exploratory plot. It is like 
## a two dimentional histogram and it works by using color
